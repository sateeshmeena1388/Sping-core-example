...
public EmployeeDTO searchEmpByNo(int no){
	...
	EmployeeDTO dto=new EmployeeDTO(ebo.getempno(),ebo.getEname(),ebo.getJob(),ebo.getSal());
	if(dto!=null)
		return dto;
	else
		return null;
...
public List<EmployeeDTO> searchEmpByDesg(String desg){
	...
	for(EmployeeBO bo:listbo){
		EmployeeDTO dto= new EmployeeDTO(bo.getempno(),bo.getEname(),bo.getJob(),bo.getSal());
